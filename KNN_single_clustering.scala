...
object KNN_single_clustering {
  def main(args: Array[String]): Unit = {
    ...
    val df_reviews_business = df_business.filter(df_business("categories").contains("Mexican")).rdd.map(x => (x(2).toString, x(13).toString)).collectAsMap()
    println("filtered")
    ...
    val train_business_user_pairs = df_reviews.map{ case(user_id:String, (business_id:String, stars:String))=>((business_id, user_id),stars)}.collectAsMap()

    //user stars
    val df_users = sqlContext.read.json("/Users/viralithakkar/Desktop/INF553/Project/Yelp_Updated_Data/user.json")
    val df_reviews_users = df_users.filter(df_users("user_id") isin(df_review_new:_*)).rdd.map(x => (x(20).toString, x(0).toString)).collectAsMap()
    ...
    val final_result = new mutable.HashMap[(String, String), Double]()
    user_business_predicted.foreach{key =>
      if (!train_business_user_pairs.contains(key._1))
        final_result.put(key._1,key._2)
    }
    val potential_users = final_result.groupBy(_._1._1).map{case (business_id:String, x:mutable.HashMap[(String, String), Double]) => (business_id, x.toList.sortBy(_._2).take(300).toMap)}
    val top_potential_users = potential_users.map{ case(business_id:String, x:Map[(String, String), Double]) => x}
...
//    }
    ...
    out.write("\n")
    top_potential_users.toList.foreach { key1 =>
      key1.foreach { key =>
